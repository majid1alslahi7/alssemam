...
export default function EditAdPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    is_featured: false, is_urgent: false, image: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { error } = await supabase.from('classified_ads').update({ ...formData, image: imageUrl }).eq('id', id);
      if (error) throw error;
      alert('تم تحديث الإعلان بنجاح'); router.push('/admin/ads');
    } catch (error) { alert('حدث خطأ في تحديث الإعلان'); } finally { setSubmitting(false); }
  };
  ...
  return (
    <div>
      <div className="mb-6"><h1 className="text-2xl font-bold flex items-center gap-2"><FaBullhorn className="text-orange-500" /> تعديل الإعلان</h1></div>
